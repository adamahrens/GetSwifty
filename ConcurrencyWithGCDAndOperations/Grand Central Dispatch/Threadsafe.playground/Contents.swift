...
//  // Person class is not threadsafe. Multiple threads are allowed to change it.
...
final class ThreadSafePerson: Person {
  // TODO: Write this implementation
  let barrierQueue = DispatchQueue(label: "com.appsbyahrens.threadsafe.person", attributes: .concurrent)
  
  // Make it a barrier queue operation
  // Meaning allows all reads to occur. Then during the write phase everything is blocked
  // until it finishes
  override func changeName(firstName: String, lastName: String) {
    barrierQueue.async(flags: .barrier) {
      
      // This is the barrier task that must finish
      super.changeName(firstName: firstName, lastName: lastName)
    }
  }
  
  override var name: String {
    // Sync to ensure all writes have occurred before accessing the variable
    barrierQueue.sync {
      super.name
    }
  }

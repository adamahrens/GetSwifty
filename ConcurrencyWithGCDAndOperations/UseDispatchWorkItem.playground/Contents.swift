...
// TODO
print(">> Create task1  and task2 as a DispatchWorkItem")
...
userQueue.async(execute: firstWorkItem)
userQueue.async(execute: secondWorkItem)
...
// TODO
if firstWorkItem.wait(timeout: .now() + 3) == .timedOut {
  print("Tired of waiting!!!!")
} else {
  print("firstWorkItem completed")
}
...
let updateUserInterfaceWorkItem = DispatchWorkItem {
  task2()
  whichQueue(workItem: "updateUserInterfaceWorkItem")
}
...
backgroundWorkItem.notify(queue: mainQueue, execute: updateUserInterfaceWorkItem)
...
if updateUserInterfaceWorkItem.isCancelled == false {
  updateUserInterfaceWorkItem.cancel()
}
// TODO
